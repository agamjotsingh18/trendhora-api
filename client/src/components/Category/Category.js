...
const Category = (props) => {
  ...
  const handleShowMore = () => {
    ...
    setTimeout(() => {
      ...
      setLoading(false);
    }, 800); // fake delay to show loading (UX improvement)
  };
    ...
    if (show !== "All") {
      ...
      items = items.filter((item) => {
        switch (show) {
          ...
          case "Accessories":
            return ["bag", "watch", "jewelry", "belt", "hat", "scarf"].includes(
              item.type?.toLowerCase()
            );
          case "Shoes":
            return ["shoes", "sneakers", "boots", "sandals"].includes(
              item.type?.toLowerCase()
            );
          case "Sale":
